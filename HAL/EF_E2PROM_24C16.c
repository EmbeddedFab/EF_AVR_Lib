...
void EF_void_EEPROM24C16_Write_Byte (U8_t memory_location, U8_t slave_address, U8_t data)
{
	...
	EF_void_I2C_Write(slave_address);     /* Send Slave address with Operation Write */
	EF_void_I2C_Write(memory_location);   /* Send Memory Location Address 			 */
	EF_void_I2C_Write(data);              /* Send Data One byte 					 */
	EF_void_I2C_Stop();
	_delay_ms(15);
}
...
U8_t EF_void_EEPROM24C16_Read_Byte (U8_t memory_location, U8_t slave_address)
{
	...
	EF_void_I2C_Write(slave_address | 1);     /* Send Slave address with Operation read  */
	data = EF_U8_I2C_Read_Byte(TRUE);
	EF_void_I2C_Stop();
